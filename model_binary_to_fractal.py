...
#
#  the program will then create a folder called 'Fractal' containing lots of good things! :)
...
for r in runs:
    for alpha in alphas:
        Path(directory+f'/Fractal/log E0 graph alpha {alpha}/Run {r}').mkdir(parents=True, exist_ok=True)
        Path(directory+f'/Fractal/Regression Plots E0/Run {r}').mkdir(parents=True, exist_ok=True)
        
        if do_1st_hom:
            Path(directory+f'/Fractal/log E1 graph alpha {alpha}/Run {r}').mkdir(parents=True, exist_ok=True)
            Path(directory+f'/Fractal/Regression Plots E1/Run {r}').mkdir(parents=True, exist_ok=True)
...
for key in pond_image:
    I = pond_image[key].shape[0]
    J = pond_image[key].shape[1]
    flatlist=[]
    for i in range(I):
        for j in range(J):
            if pond_image[key][i,j]:
                flatlist.append([j,i])
...
for r in runs:
    for s in steprange:
        ...
        for a in alphas:
            ...
            os.chdir(directory+f'/Fractal/log E0 graph alpha {a}/Run {r}')
            np.save(f'Run {r} Step {s} log E0 graph alpha {a}.npy',log_E0_graph[a,r,s])

            if do_1st_hom:
                log_E1_graph[a,r,s]=np.log(E1_graph[a,r,s])
                os.chdir(directory+f'/Fractal/log E1 graph alpha {a}/Run {r}')
...
for r in runs:
    for s in steprange:
        ...
        os.chdir(directory+f'/Fractal/Regression Plots E0/Run {r}')
        ...
        if do_1st_hom:
            ...
            os.chdir(directory+f'/Fractal/Regression Plots E1/Run {r}')
...
fractal_dim.to_pickle(directory+'/Fractal/Fractal Dimensions.pkl')
